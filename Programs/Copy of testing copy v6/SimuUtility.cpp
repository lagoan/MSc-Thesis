...
void CSimuUtility::AssertSimuValue(SimuValue value)
{
...
void CSimuUtility::AssertSimuVector3D(CVector3D* vector)
{
	for (UINT i=0; i < 3; i++)
		AssertSimuValue(vector->v[i]);
}

bool CSimuUtility::FileDoesExist(CString fileName)
{
	FILE* fp;
...
{
	SimuValue volume = pow(radius, 3)*4*PI/3;
	return volume;
}
...
void CSimuUtility::DrawCircleOnZ0ByOpenGL(SimuValue radius, SimuValue height,
										  SimuValue segments)
{
	SimuValue dang = PI*2/segments;
	glBegin(GL_LINE_LOOP);
	for(SimuValue ang=0; ang <= 2*PI; ang += dang)
...
{
	SimuValue s = sin(rAngle/180*PI);
	SimuValue c = cos(rAngle/180*PI);
	SimuValue t = 1 - c;
...
int CSimuUtility::SolveCubicEquation(SimuValue coeff[4], SimuValue x[3])
{
	...
	// One real root
		...
		SimuValue e = pow(sqrt(-d) + fabs(R), 1. / 3.);
		if (R > 0)
			e = -e;
		x[0] = (e + Q / e) - a1 / 3.;
		return 1;
...
SimuValue CSimuUtility::SineWeightFunction(SimuValue distance, SimuValue radius)
{
	...
	if (distance >= 0 && distance <= radius)
	{
		weight = -SIMU_PARA_SINE_KERNEL_NORMALIZER*cos(2*PI*distance/radius);
	}
	// else weight = 0
	return weight;
}
														  ...
														  SimuValue radius)
{
	...
	if (distance >= 0 && distance <= radius)
	{
		weight = SIMU_PARA_SINE_KERNEL_NORMALIZER*sin(2*PI*distance/radius)*2*PI/radius;
	}
	// else weight = 0
	return weight;
}
														   ...
														   SimuValue radius)
{
	...
	if (distance >= 0 && distance <= radius)
	{
		weight = SIMU_PARA_SINE_KERNEL_NORMALIZER*cos(2*PI*distance/radius)*4*PI*PI/(radius*radius);
	}
	// else weight = 0
	return weight;
}
...
SimuValue CSimuUtility::SplineGradientWeightFunction(SimuValue distance, SimuValue radius)
{
	...
	if (q >= 0 && q < 1)
	{
		weight = -3*q + 2.25*pow(q, 2);
	}
	else if (q >= 1 && q < 2)
	{
		weight = -0.75*pow((2-q), 2);
	}
	else // weight = 0
		return weight;
...
//		weight *= SIMU_CONST_SPIKY_NORMALIZER/pow(radius, 7);
		// original formula
		weight = -3*pow(s, 2);
		weight *= SIMU_CONST_SPIKY_NORMALIZER/pow(radius, 6);
//		weight = -s;
...
#define		SIMU_PARA_POLY_KERNEL_NORMALIZER	315/(64*PI)
SimuValue CSimuUtility::PolyFirstDerivativeWeightFunction(SimuValue distance, SimuValue radius)
{
	SimuValue weight = 0;
	if (distance < radius)
	{
		weight = -6*distance*pow(radius*radius - distance*distance , 2);
...
#define		SIMU_PARA_VISC_KERNEL_NORMALIZER	7.5/PI
...
SimuValue CSimuUtility::AngleBetween2Vectors(CVector3D* pV0, CVector3D* pV1)
{
	...
	return atan2(crossProduct.Length(), dotProduct)*180/PI;
}
