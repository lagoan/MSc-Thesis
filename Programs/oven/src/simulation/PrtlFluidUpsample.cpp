...
void CPrtlFluidUpsample::DetectSrfcPrtls()
{
	...
	for (int i=0; i < m_pfPrtls.m_paNum; i++)
	{
		...
		if (IsPrtlStatusDefinite(pPrtl, m_pfuSrfcPrtls))
			continue;
		pPrtl->ComputePrtlNumDenAndGradLen(pPrtl->m_pfSmoothLen, pnd, gradLen);
...
bool CPrtlFluidUpsample::HasAnySrfcPrtlsInNgbrhood(CVector3D* pos, SimuValue radius)
{
	...
	kdTree.rangeSearch(pos, CSimuManager::m_prtlDistance, possibleNgbrPrtls);
...
//	sprintf(tmpStr, " minDR=%.2f", m_pfuMinPrtlDistRatio); 
	m_pfSimuInfo += tmpStr;
	if (m_pfuDownSampled)
	{
		m_pfuDownSampled = false;
											  ...
											  bool bMergeIntoFirst)
{
	...
	newPrtl->m_pfSmoothLen = (pPrtl0->m_pfSmoothLen + pPrtl1->m_pfSmoothLen) / 2.0f;
	...
	kdTree.rangeSearch(newPrtl->m_vpPos, newPrtl->m_pfSmoothLen, possibleNgbrPrtls);
	newPrtl->InterpolatePrtlValues(possibleNgbrPrtls, newPrtl->m_pfSmoothLen);
	...
	kdTree.rangeSearch(newPrtl->m_vpPos, newPrtl->m_pfSmoothLen, ngbrPrtls);
											   ...
											   TGeomElemArray<CFluidPrtl> &ngbrPrtls)
{
	...
	kdTree.rangeSearch(pPos, smoothLength, possibleNgbrPrtls);
	SimuValue* weights = new SimuValue[possibleNgbrPrtls.m_paNum]; 
	SimuValue totalWeight = 0;
	for (int i=0; i < possibleNgbrPrtls.m_paNum; i++)
	{
