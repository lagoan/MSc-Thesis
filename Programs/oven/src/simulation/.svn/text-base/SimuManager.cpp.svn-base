...
int CSimuManager::m_numFluidBubbleInteractionFrequency = 1;

// modeling options
int CSimuManager::m_numFramesPerSecond		= 24;
int CSimuManager::m_numStepDivPerFrame		= 4;
int CSimuManager::m_maxPoissonPauseSteps	= 1;
int CSimuManager::m_numTensorSubIntegraions = 1;
int CSimuManager::m_maxSrfcDetectPauseSteps	= 1; //5; // ?
...
CSimuManager::EFluidModel CSimuManager::m_eFluidModel = SPH;
...
ERotation CSimuManager::m_eRotationDerivative =  NO_ROTATION; //COROTATIONAL; //NO_ROTATION; // // COROTATIONAL XXX
SimuValue CSimuManager::m_rotationFactor	= 10;
bool CSimuManager::VISCOSITY				= false;
SimuValue CSimuManager::m_viscosity			= 500000;//1.0;//50000;//500000;//100000;//1.6;//1; // molases
...
SimuValue CSimuManager::m_interFluidDamping	= 0.0; // range 0 to 1, and 0 means complete damping
SimuValue CSimuManager::m_interFluidTension	=  1000000;//1000;
...
SimuValue CSimuManager::m_shearModulus		= pow(10.0f, 3);
...
// baking parameters
bool	  CSimuManager::BAKE_FLUIDS				= true;
SimuValue CSimuManager::m_startCookTime = 0;
//SimuValue CSimuManager::m_CO2PeakGeneration		= 4.0f;
//SimuValue CSimuManager::m_CO2TemperatureMean	= 40.0f;
//SimuValue CSimuManager::m_CO2TemperatureRange	= 10.0f;
//SimuValue CSimuManager::m_CO2AreaUnderRateCurve = 0.0f;


// thermal model parameters

bool CSimuManager::AMBIENT_HEAT_TRANSFER	= true;
//bool CSimuManager::GENERATE_CO2				= true;

bool CSimuManager::HEAT_TRANSFER			= false;
bool CSimuManager::NOT_CONSERVE_HEAT		= false;
bool CSimuManager::OVERWRITE_TEXTURE		= false;
int CSimuManager::m_subThermalSteps			= 1;// 3; //1;
...
SimuValue CSimuManager::m_maxShearModulus	= pow(10.0f, 9);
...
bool CSimuManager::m_bFixedParticles		= false;
...
QString	CSimuManager::m_strAppDataDir		= "";
QString CSimuManager::m_strImportDir		= "";
QString CSimuManager::m_strImportParaFile	= "";
unsigned int CSimuManager::m_nImportFrameInterval	= CSimuManager::m_numFramesPerSecond/32;
...
QString CSimuManager::m_strExportDir		= "";
QString CSimuManager::m_strSimuInfoFile		= "";
QString CSimuManager::m_strSimuEnvPOVFile	= "SimuEnv.pov";
bool CSimuManager::m_bNeedExportDirectory	= true;
unsigned int CSimuManager::m_nCurFrame				= 1;
SimuValue CSimuManager::m_fPreExportTime	= SIMU_NEGATIVE_NUMBER; // so that the first frame will be exported
QString CSimuManager::m_strDataDebugFile	= "dataDebugFile.txt";
FILE* CSimuManager::m_dataDebugFile			= NULL;
bool CSimuManager::m_bWriteToDataDebugFile	= false;

// animation monitor variables
unsigned int CSimuManager::m_nCurStep				= 0;
...
time_t CSimuManager::GetDirectoryCreationTime(QString dir)
{
	return 0;
}
...
void CSimuManager::OpenDataDebugFile()
{
	...
	QString dataDebugFile = CSimuManager::m_strAppDataDir;
	dataDebugFile += "\\";
	dataDebugFile += CSimuManager::m_strDataDebugFile;
	if ((CSimuManager::m_dataDebugFile = fopen(dataDebugFile.toAscii(), "a")) == NULL)
	{
...
{
	if (CSimuMsg::strSimuInfo.size() <= maxMsgLen)
		return;
	QString newSimuInfo = "";
	int i;
...
void CSimuManager::DisplayOperationCountingMessage(const char* elmntName,
													unsigned int countNum, unsigned int totalNum,
													unsigned int period,	// default = 100
...
void CSimuManager::InitNamedVector(CVector* vector)
{
	...
	else if (m_cParaList.contains(vector->GetName()))
...
//	//TRACE("Draw Simulation Bodies\r\n");
	...
	for (i=0; i < m_aryPrtlFluid.m_paNum; i++)
	{
...
{
	// XXX may need to change to SimuManager
	...
	m_bGo	= false;
	m_uFwdIntvls		= 0;
	m_dbAnimationClock	= 0;
	m_dbTimeStep = ComputeTimeStep();
										  ...
										  TGeomElemArray<CFluidPrtl> &pNgbrs)
{
	for (int i=0; i < m_aryPrtlFluid.m_paNum; i++)
	{
		...
		pFluid->kdTree.rangeSearch(pPrtl->m_vpPos, pPrtl->m_pfSmoothLen, pNgbrs);
	}
}
...
CPrtlFluid* CSimuManager::CreatePrtlFluid()
{
	...
	pPrtlFluid->m_pbFixedParticles = CSimuManager::m_bFixedParticles;
...
{
	if (m_curPrtlFluid != NULL)
		m_curPrtlFluid->m_pfTimeStep = m_dbTimeStep;
}
...
/////////////////////////////////////////////////////////////////////////////
bool CSimuManager::DelimiteNameValuePair(char* src, QString& name, QString& string)
{
...
void CSimuManager::ReadSimulationParameters()
{
	...
	QString	strFileRef(m_strAppDataDir);
	strFileRef += "\\";
	...
	if ((fp=fopen(strFileRef.toAscii(),"r")) == NULL)
	{
...
void CSimuManager::ImportFluidParameters()
{
	...
	QString fileName = m_strImportDir;
	fileName += "\\";
	...
	else
	{
		...
		float tmpFloat;
		fscanf(fp, "frame interval=%f\n", &tmpFloat); m_dbFrameIntvl = tmpFloat;
		fclose(fp);
	}
}
...
void CSimuManager::ExportFluidParameters()
{
	...
	FILE* fp;
	// make the fluid info file
	QString fileName = m_strExportDir;
	fileName += "\\";
	fileName += APP_FLUID_PARA_FILE_NAME;
	// Open the fluid info file
	if ((fp=fopen(fileName.toAscii(), "w")) == NULL)
	{
	...
	switch (CSimuManager::m_eFluidClass)
	{
		case PrtlFluid:
			if (m_curPrtlFluid != NULL)
			{
				FILE* fp;
				// make the fluid info file
				QString fileName = CDataExImport::MakePOVRayDataFileName(m_strImportDir,
			...
			FILE* POVRayFp;
			QString fileName = CDataExImport::MakePOVRaySceneFileName(m_strImportDir,
																		m_nCurFrame);
			// Open povray file
		...
		switch (CSimuManager::m_eFluidClass)
		{
			case PrtlFluid:
				if (m_curPrtlFluid != NULL)
				{
					FILE* fp;
					// make the fluid info file
					QString fileName = CDataExImport::MakePOVRayDataFileName(m_strExportDir,
													m_nCurFrame, m_curPrtlFluid->m_deiName);
					// Open the fluid info file
				...
				// make povray file name
				FILE* POVRayFp;
				QString fileName = CDataExImport::MakePOVRaySceneFileName(m_strExportDir,
																			m_nCurFrame);
				// Open povray file
				...
				for (int i=0; i < m_aryPrtlFluid.m_paNum; i++)
				{
					...
					QString fileNamePrtlSpheres = pFluid->MakePrtlDataFileName(m_nCurFrame);
					fprintf(POVRayFp, "\t#include \"%s\"\n", fileNamePrtlSpheres);
					fprintf(POVRayFp, "#else\n");
					QString fileNameSurfaceMesh = pFluid->MakeSurfaceMeshDataFileName(m_nCurFrame);
					fprintf(POVRayFp, "\t#include \"%s\"\n", fileNameSurfaceMesh);
...
bool CSimuManager::OnCmdMsg(unsigned int nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
...
// XXX necessary (?)
void CSimuManager::OnTimer(unsigned int nIDEvent) 
{
...
//	CFrameWnd::OnTimer(nIDEvent);
	...
	{
		CSimuMsg::ExitWithMessage("CSimuManager::OnTimer(unsigned int nIDEvent)",
									"Can not kill timer.");
		return;
	}
	m_pSimuMonitor->UpdateData();
