...
SurfaceMesher::SurfaceMesher()
{
	...
	if (pluginLoader.isLoaded()) {
		//std::cout << "YEAH!!!! loaded the mofo\n";
	} else {
		//std::cout << "plugin loader FAIL\n";
	}
	if (mesherPluginLoader.isLoaded()) {
		//std::cout << "YEAH!!!! loaded the mofo mesher\n";
	} else {
		//std::cout << "plugin loader FAIL mesher\n";
	}	
	...
	//std::cout << "one and\n";
	QString actionName = surfaceFilter->filterName(FP_POISSON_RECON);
	QAction POISSON_ACTION(actionName, NULL);
	
	MeshDocument temp;
	
	surfaceFilter->initParameterSet(&POISSON_ACTION, temp, filterParameters);
...
/*
void SurfaceMesher::prepareGTSNSurface(GtsSurface * s1) 
{
...
void SurfaceMesher::prepareGTSSurface(GtsSurface * s1) 
{
	s1 = gts_surface_new (gts_surface_class (),
							  gts_face_class (),
							  gts_edge_class (),
								 ...
								 Halfedge *edge)
{
	...
	////std::cout << "\t\t "<< edgesList.size() << " NOT FOUND !!!\n";
	return 0;
}

#if 0
QString SurfaceMesher::getEdgeKey(Halfedge &edge)
{
	Point_3 p1 = edge.vertex()->point();
	Point_3 p2 = edge.prev()->vertex()->point();

	double squared1 = CGAL::to_double(p1.x()*p1.x() + 
									  p1.y()*p1.y() + 
									  p1.z()*p1.z());
	
	double squared2 = CGAL::to_double(p2.x()*p2.x() + 
									  p2.y()*p2.y() + 
									  p2.z()*p2.z());
	
	if (squared1 > squared2) {
		p1 = edge.prev()->vertex()->point();
		p2 = edge.vertex()->point();
	} 
	
	return SurfaceMesher::getPointKey(p1) + 
		   QString("-") + 
		   SurfaceMesher::getPointKey(p2);
...
//				myBox.max.X();
				...
				//double isoDensityRadius = 0.7; // max value
				
				// bunny with very well defined particles
				//double isoDensityRadius = 0.2; // max value
				//double isoThreshold = 1;
				
				
				// fluid with random particles 
				
				//double isoDensityRadius = 0.15; // max value
				//double isoThreshold = .6;
				
				//double isoDensityRadius = 0.2; // max value
				//double isoThreshold = .5;
				
				// for cake
				//double isoDensityRadius = 0.6; // max value
				//double isoDensityRadius = 0.15; // max value
				double isoDensityRadius = 0.15; // max value
				double isoThreshold = .3;
				
				isoDensityRadius = .1 * 5;
				//isoDensityRadius = .1 * 1.5;
				isoThreshold = .1; //.2

					// isoDensityRadius = .1 * 3.0;
					// 				//isoDensityRadius = .1 * 1.5;
					// 				isoThreshold = .2; //.2
					// 			
					// 
				...
				//QHash<QString, bool> negativeSpace;
				//std::vector< std::vector< std::vector<bool> > > negativeSpace;
				bool *** negativeSpace;
				//QHash<QString, std::pair<VoxelCoord, double> > toAverageValues;
				//std::map<VoxelCoord, double> toAverageValues;
				std::vector< std::pair<VoxelCoord, double> > toAverageValues;
				//std::map<VoxelCoord, double>::iterator avrgValIt;
			      		...
			      		for(int k=0;k<xGridD;k++) {
							...
							for (rangeIterator = range.begin();
								 ...
								 ++rangeIterator) {
									...
									currentDelimiter = .3 + (currentDelimiter * .3);
									//currentDelimiter = .1;
									...
									// 								//std::cout << "currentDelimiter " << currentDelimiter << "\n";
									//currentDelimiter = .15 + (currentDelimiter * .05);
									//currentDelimiter = .1 + (currentDelimiter * .2);
									//currentDelimiter = .05 + (currentDelimiter * .2);
									////std::cout << currentDelimiter << "\n";
									
									//BUENERO
									currentDelimiter = .3 + (currentDelimiter * .3);
									//currentDelimiter = 2 + (currentDelimiter * .3);
									
									//currentDelimiter = .25 + (currentDelimiter * .25);
