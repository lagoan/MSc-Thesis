...
CSimuMarchingCube::CSimuMarchingCube()
{
	...
	double exp10 = log10( m_prtlDistance * 0.001);
	int numDigits;
	if (exp10 > 0)
		numDigits = 1;
	else
...
void CSimuMarchingCube::DrawMarchingCubeSurfaceMeshByOpenGL(CLinearTemperaTexture* pLTT)
{
	...
	{
		double normalTipX, normalTipY, normalTipZ;
		glBegin(GL_LINES);
		for (int i=0; i < m_srfcPs.size(); i++)
		{
	...
	{
		glBegin(GL_LINES);
		for (int i=0; i < m_srfcTris.size(); i++)
		for (int n=0; n < 3; n++)
		{
	...
	else
	{
		...
		for (int i=0; i < m_srfcTris.size(); i++)
		{
			...
			{
				double triTemperature = m_srfcPointTemperatures[pointIds[0]];
...
void CSimuMarchingCube::ImportVertexPositions(FILE* fp)
{
	...
	fscanf(fp, "%d,\n", &number);
	m_srfcPs.resize(number);
	int i;
	char tmpStr[LEN_DATA_LINE];
	float temperature;
	for (i=0; i < m_srfcPs.size(); i++)
	{
		fgets(tmpStr, LEN_DATA_LINE, fp);
		...
		if (temperatureStr != NULL)
		{
			if (i == 0)
			{
				if (m_srfcPointTemperatures != NULL)
					delete m_srfcPointTemperatures;
				m_srfcPointTemperatures = new double[m_srfcPs.size()];
			}
...
{
	DeleteSurfaceElements();
	InitializeTempMemory(gridDim[0], gridDim[1], gridDim[2]);
...
*/	
	m_data = new double[totalGridPoints];
...
double CSimuMarchingCube::GetDataValue(int idX, int idY, int idZ)
{
...
int CSimuMarchingCube::AddSrfcPointOnX()
{
	...
	CSimuSrfcPoint3D newP;

	double u = ( m_v[0] - 0) / ( m_v[0] - m_v[1] ) ;
	...
	m_srfcPs.push_back(newP);
	
	return m_srfcPs.size()-1 ;
}
...
double CSimuMarchingCube::GetGradOnX(int idX, int idY, int idZ)
{
  if( idX > 0 )
  {
    if ( idX < m_sizeX - 1 )
...
double CSimuMarchingCube::GetGradOnY(int idX, int idY, int idZ)
{
  if( idY > 0 )
  {
    if ( idY < m_sizeY - 1 )
...
double CSimuMarchingCube::GetGradOnZ(int idX, int idY, int idZ)
{
  if( idZ > 0 )
  {
    if ( idZ < m_sizeZ - 1 )
...
bool CSimuMarchingCube::TestCubeFace(char face)
{
	...
	case -6 : case 6 :  A = m_v[4] ;  B = m_v[7] ;  C = m_v[6] ;  D = m_v[5] ;  break ;
		default : std::cout << "Invalid face code\n";
			
			//CSimuMsg::ExitWithMessage(location, "Invalid face code\n") ;  A = B = C = D = 0 ;
	};
...
bool CSimuMarchingCube::TestCubeInterior(char s)
{
	...
	case 11 :
	  ...
	  break ;
		default : std::cout << "Invalid edge\n" ; break;
			//CSimuMsg::ExitWithMessage(location, "Invalid edge\n") ;  break ;
	}
	break ;

		default :  std::cout <<"Invalid ambiguous case\n"; break;
			//CSimuMsg::ExitWithMessage(location, "Invalid ambiguous case\n") ;  break ;
	}
